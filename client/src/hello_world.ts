...
*/
const PROGRAM_SO_PATH = path.join(PROGRAM_PATH, 'seeded_contract.so');

/**
  * Path to the keypair of the deployed program.
  * This file is created when running `solana program deploy dist/program/helloworld.so`
*/
const PROGRAM_KEYPAIR_PATH = path.join(PROGRAM_PATH, 'seeded_contract-keypair.json');
...
export async function checkProgram(): Promise<void> {
  ...
  try {
    ...
    throw new Error(
      `Failed to read program keypair at '${PROGRAM_KEYPAIR_PATH}' due to error: ${errMsg}. Program may need to be deployed with \`solana program deploy dist/program/seeded_contract.so\``,
    );
  }
  ...
  if (programInfo === null) {
    if (fs.existsSync(PROGRAM_SO_PATH)) {
      throw new Error('Program needs to be deployed with `solana program deploy dist/program/seeded_contract.so`');
    } else {
